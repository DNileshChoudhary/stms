...
function App() {
  ...
  useEffect(() => {
    ...
    // Existing User — set admin status on page refresh
    if (savedUser) {
      ...
      if (savedUser === "nilesh") {
        ...
        setIsAdmin(false); // ✅ explicitly false for non-admin users
      }
    }
    ...
    if (token) {
      ...
      if (oauthUsername === "nilesh") {
        ...
        setIsAdmin(false); // ✅ explicitly false for non-admin OAuth users
        setUsers([]);
      }
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API}/admin/users`, {
  ...
  const login = async () => {
    try {
      ...
      // ✅ Set admin only for the admin user, explicitly false for everyone else
      if (username === "nilesh") {
        setIsAdmin(true);
        fetchUsers();
      } else {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API}/admin/analytics`, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
      });
  ...
  const deleteUser = async (id) => {
    ...
    try {
      await axios.delete(`${process.env.REACT_APP_API}/admin/users/${id}`, {
        withCredentials: true,
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
  ...
  const askAI = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API}/ai`, {
        params: {
          task: title,
        },
      });
  ...
  const generateSchedule = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API}/ai/schedule`, tasks);
        ...
        {!isLoggedIn ? (
          ...
          <>
            ...
            <div className="dashboard-main">
              ...
              <h2 className="welcome-text">Hello, {loggedInUser} 👋</h2>
              ...
              {/* ✅ ADMIN DASHBOARD — only renders when isAdmin is true */}
              {isAdmin && (
                <div className="admin-box">
                  ...
                  <div className="userCount">
                    ...
                    {analytics && (
                      <div className="stats">
                        <div className="card">
                          👥<h3>{analytics.users}</h3>Users
                        </div>
                        <div className="card">
                          📝<h3>{analytics.tasks}</h3>Tasks
                        </div>
                        <div className="card">
                          ✅<h3>{analytics.completed}</h3>Completed
                        </div>
                        <div className="card">
                          ⏳<h3>{analytics.pending}</h3>Pending
                        </div>
                      </div>
                    )}
                  </div>
                            ...
                            {(provided) => (
                              ...
                              >
                                <div className="task-left">
                                  ...
                                  <div className="taskDate">📅 {task.dueDate}</div>
